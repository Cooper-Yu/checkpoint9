...
public:
        ...
        target_base_frame_("robot_base_link")
  {
    ...
    approach_service_ = create_service<attach_shelf::srv::GoToLoading>(
        "/approach_shelf",
        std::bind(&ApproachServiceServer::handle_approach_request, this, std::placeholders::_1,
                  std::placeholders::_2),
        rmw_qos_profile_services_default, service_callback_group_);

    RCLCPP_INFO(get_logger(),
                "approach_service_server ready on /approach_shelf; target_base_frame=%s",
...
private:
    ...
    while (rclcpp::ok() && (now() - start_time).seconds() < movement_timeout_) {
      ...
      if (!next_cart_frame.has_value() && smooth_forward && step >= center_lock_min_steps_) {
        ...
        RCLCPP_WARN(get_logger(),
                    ...
                    center_drive_scale_, locked_drive_distance);
        if (!drive_forward_open_loop(
                locked_drive_distance,
  ...
  bool drive_forward_open_loop(double distance, const std::string & label, bool stop_after = true)
  {
    ...
    RCLCPP_INFO(get_logger(), "%s: distance=%.3f m, speed=%.3f m/s, duration=%.3f s, stop_after=%s",
                label.c_str(), distance, forward_speed_, drive_time, stop_after ? "true" : "false");
